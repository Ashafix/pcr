...
import psutil
import time
from cgi_bin.support_functions import read_aws_conf
...
def main():
    ...
    time.sleep(1800)
    while True:
        file_list = [(os.path.getmtime(data_dir + fn), os.path.basename(data_dir + fn)) for fn in os.listdir(data_dir)]
        file_list.sort()
        ...
        if ((time.time() - newest) / 60) > 50:
            ...
            if int(psutil.cpu_percent()) < 10:
                ...
                compute_host.stop(DryRun=False, Force=False)
